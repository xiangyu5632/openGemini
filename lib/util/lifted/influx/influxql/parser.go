...
2022.01.23 Add snapshot statement, HINT grammar.
...
const (
	...
	// if the tag is full, the index get the SID based on seriesKey, otherwise, the index work as normal procedure.
	SpecificSeriesQuery = "specific_series"

	// Whether the tag is full or not, the index get the SID based on seriesKey.
	FullSeriesQuery = "full_series"
...
Loop:
	for {
		...
		switch tok {
		case SHARD:
			stmt.ShardGroupDuration, err = p.parseRelatedDuration()
			if err != nil {
				return nil, err
			}
		case HOT:
			stmt.HotDuration, err = p.parseRelatedDuration()
			if err != nil {
				return nil, err
			}
		case WARM:
			stmt.WarmDuration, err = p.parseRelatedDuration()
			if err != nil {
				return nil, err
			}
		case INDEXCOLD:
...
func (p *Parser) parseRelatedDuration() (time.Duration, error) {
	if tok1, pos, lit := p.ScanIgnoreWhitespace(); tok1 != DURATION {
		return 0, newParseError(tokstr(tok1, lit), []string{"DURATION"}, pos)
	}
	if tok1, pos, _ := p.ScanIgnoreWhitespace(); tok1 == INF {
		return 0, &ParseError{
			Message: "invalid duration INF for shard/hot/warm/index duration",
			Pos:     pos,
		}
	}
	p.Unscan()
...
func (p *Parser) haveHints() bool {
	rd, _ := p.s.s.r.r.(*bufio.Reader)
	if rd == nil {
		return false
	}
	line, _ := rd.Peek(2048)
	for i := 0; i < len(line)-3; i++ {
		...
		if bytes.Compare(line[i:i+3], []byte("/*+")) != 0 {
			return false
		} else {
			idx := bytes.Index(line[i+3:], []byte("*/"))
			if idx > 0 {
				return true
			}
		}
...
func (p *Parser) parseCreateDatabaseStatement() (*CreateDatabaseStatement, error) {
	...
	Loop:
		for {
			...
			case SHARD:
				stmt.RetentionPolicyShardGroupDuration, err = p.parseRelatedDuration()
				if err != nil {
					return nil, err
				}
			case HOT:
				stmt.RetentionPolicyHotDuration, err = p.parseRelatedDuration()
				if err != nil {
					return nil, err
				}
			case WARM:
			...
			case INDEX:
				stmt.RetentionPolicyIndexGroupDuration, err = p.parseRelatedDuration()
				if err != nil {
					return nil, err
				}
			case NAME:
			...
			default:
				if len(found) == 0 {
					return nil, newParseError(tokstr(tok, lit),
						[]string{"DURATION", "REPLICATION", "SHARD", "NAME", "INDEX", "HOT", "WARM"}, pos)
				}
				p.Unscan()
				break Loop
			}
...
func (p *Parser) parseCreateMeasurementStatement() (*CreateMeasurementStatement, error) {
	stmt := &CreateMeasurementStatement{Type: HASH}

	// Parse the name of the measurement to be created.
	idents, err := p.parseSegmentedIdents()
	if err != nil {
		return nil, err
	}
	...
	if len(idents) == 3 {
		stmt.Database, stmt.RetentionPolicy, stmt.Name = idents[0], idents[1], idents[2]
	}
	...
	case 1:
		stmt.Name = idents[0]
	case 2:
		stmt.RetentionPolicy, stmt.Name = idents[0], idents[1]
	}
	...
	if tok, _, _ := p.ScanIgnoreWhitespace(); tok == WITH {
		...
		/*if tok != SHARDKEY {
			return nil, newParseError(tokstr(tok, lit), []string{"SHARDKEY"}, pos)
		}*/
		// rewind
		p.Unscan()

		if err := p.parseTokens([]Token{SHARDKEY}); err != nil {
			p.Unscan()
		} else {
			stmt.ShardKey, err = p.ParseIdentList()
			if err != nil {
				return nil, err
			}
		...
		if err := p.parseTokens([]Token{TYPE}); err != nil {
			p.Unscan()
		} else {
			stmt.Type, err = p.ParseIdent()
			if err != nil {
				return nil, err
			}
			if stmt.Type != RANGE && stmt.Type != HASH {
				return nil, newParseError(stmt.Type, []string{RANGE, HASH}, pos)
			}
		}
	} else {
		p.Unscan()
...
func (p *Parser) parseAlterShardKeyStatement() (*AlterShardKeyStatement, error) {
	stmt := &AlterShardKeyStatement{Type: HASH}
...
func (p *Parser) parseCreateUserStatement() (*CreateUserStatement, error) {
	...
	if err := p.parseTokens([]Token{ALL, PRIVILEGES}); err == nil {
		...
		p.Unscan()
		return nil, errors.New("create user cmd format err")
	}
	//return nil, errors.New("create user cmd format err")
}
...
func (p *Parser) parsePrepareSnapshotStatement() (*PrepareSnapshotStatement, error) {
	return &PrepareSnapshotStatement{}, nil
}

func (p *Parser) parseEndPrepareSnapshotStatement() (*EndPrepareSnapshotStatement, error) {
	return &EndPrepareSnapshotStatement{}, nil
}

func (p *Parser) parseGetRuntimeInfoStatement() (*GetRuntimeInfoStatement, error) {
	return &GetRuntimeInfoStatement{}, nil
}
...
func (p *Parser) parseHints() (Hints, error) {
	// If the next token is not Hint then exit.
	tok, _, lit := p.ScanIgnoreWhitespace()
	if tok != HINT {
		p.Unscan()
		return nil, nil
