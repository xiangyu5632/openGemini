...
func (data *Data) RecoverData(db string, metaInfo *Data, nodeMap map[uint64]uint64) error {
	// PtView
	dbPT, ok := metaInfo.PtView[db]
	if !ok {
	...
	// ReplicaGroups
	if metaInfo.ReplicaGroups != nil {
		rpGroups, ok := metaInfo.ReplicaGroups[db]
		if ok {
	...
	// Databases
	database, ok := metaInfo.Databases[db]
	if !ok {
