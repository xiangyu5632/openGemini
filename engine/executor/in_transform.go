...
func (trans *InTransform) rewriteInCondition(inConditon *influxql.InCondition) (influxql.Expr, error) {
	var preNode influxql.Expr
	for bufVal := range trans.BufColumnMap {
		val, err := trans.GetVal(bufVal)
		if err != nil {
			return nil, err
		}
		node := &influxql.BinaryExpr{
			Op:  influxql.EQ,
			LHS: influxql.CloneExpr(trans.OuterVarRef),
			RHS: val,
		}
		if preNode != nil {
			node = &influxql.BinaryExpr{
...
func (trans *InTransform) GetVal(bufVal interface{}) (influxql.Expr, error) {
	switch trans.OuterVarRef.Type {
	case influxql.Integer:
		val, ok := bufVal.(int64)
		if !ok {
			return nil, fmt.Errorf("Get int Val from bufColumnMap err")
		}
		return &influxql.IntegerLiteral{Val: val}, nil
	case influxql.Float:
		val, ok := bufVal.(float64)
		if !ok {
			return nil, fmt.Errorf("Get float Val from bufColumnMap err")
		}
		return &influxql.NumberLiteral{Val: val}, nil
	case influxql.Boolean:
		val, ok := bufVal.(bool)
		if !ok {
			return nil, fmt.Errorf("Get bool Val from bufColumnMap err")
		}
		return &influxql.BooleanLiteral{Val: val}, nil
	case influxql.String, influxql.Tag:
		val, ok := bufVal.(string)
		if !ok {
			return nil, fmt.Errorf("Get string Val from bufColumnMap err")
		}
		return &influxql.StringLiteral{Val: val}, nil
	default:
		return nil, nil
	}
}
