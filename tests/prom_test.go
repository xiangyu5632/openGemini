...
func TestServer_PromQuery_Subquery_AvgOverTime(t *testing.T) {
	...
	writes := []string{}
	for i := 1753259400018000000; i < 1753259459382000000; i += 1000000 {
	...
	test.addQueries([]*Query{
		{
			name:    "range query: increase(subquery)",
			params:  url.Values{"db": []string{"db0"}, "start": []string{"1753259460"}, "end": []string{"1753259460"}, "step": []string{"60s"}},
			command: `avg_over_time(metric1{}[59999ms])`,
			exp:     `{"status":"success","data":{"resultType":"matrix","result":[{"metric":{},"values":[[1753259460,"14"]]}]}}`,
			path:    "/api/v1/query_range",
		},
	}...)
