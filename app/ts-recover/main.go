...
func ParseFlags(args ...string) (recover.RecoverConfig, error) {
	...
	fs.StringVar(&options.Host, "host", "127.0.0.1", "meta node host")
	fs.StringVar(&options.Port, "port", "8091", "meta node port")
	if err := fs.Parse(args); err != nil {
		return recover.RecoverConfig{}, err
	}
	if options.ConfigPath == "" {
		return options, errors.New("missing required parameter: config")
	}
	return options, nil
}
